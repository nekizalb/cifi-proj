...
const sections = [
    ...
    },
    {
        ...
        children: [
            ...
            { id: 'progress', type: 'number', label: 'To Next', text: ' ', textId: 'rankrequirement', style: { width: 70 } },
            {
                id: 'install3',
                type: 'number',
                label: '<a href="#" data-bs-toggle="tooltip" data-bs-title="Material Scavenger Vehicles" tabindex="-1">Install 3</a>',
                max: 5,
            ...
            },
            {
                id: 'install6',
                type: 'number',
                label: '<a href="#" data-bs-toggle="tooltip" data-bs-title="Academy Auto-Scrappers" tabindex="-1">Install 6</a>',
                max: 75,
...
academyEffectorPortal.pages.default.dataLinkage = {
    ...
    set earthly(value) { playerData.shardMilestones[28] = value; },
    ...
    set iapCollector(value) { playerData.diamonds.iapCollector = value; },

    get wonderous() { return playerData.shardMilestones[25]; },
    get earthly() { return playerData.shardMilestones[28]; },
    ...
    sections.forEach(({ name, children, style }) => {
        ...
        children.map(({ id, label, type, text, style = {}, ...props }) => {
            if (['select', 'number', 'checkbox'].includes(type)) {
                const group = createElement('div', 'd-flex align-items-center column-gap-2 mb-2')
                group.innerHTML = `<div class="${props.labelClassName ? props.labelClassName : 'flex-fill'}"><label class="col-form-label col-form-label-sm">${label}</label></div>`
...
academyEffectorPortal.pages.default.updateFunction = function(e) {
    ...
    if (e.target.type === 'number') {
        ...
        if (e.target.id === 'rank') {
            portalPanel['rankrequirement'].innerText = '/ ' + GameDB.fleet.zeus.rankRequirements[portalPanel.dataLinkage[e.target.id]];
        }
